...
export default function AddressProof() {
  const [fileName, setFileName] = useState<string | null>("Comprovativo_Residencia.pdf");
    ...
    <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-6 shadow-xl space-y-4 w-full flex flex-col justify-between">
      <div className="space-y-3">
        <div className="flex items-center gap-3 border-b border-slate-800 pb-3">
          ...
          <div>
            <h2 className="text-lg font-bold text-white">Comprovativo de Residência</h2>
            <p className="text-xs text-slate-400">Fatura de água, luz ou declaração de bairro (últimos 3 meses)</p>
          </div>
        </div>

        <div className="border-2 border-dashed border-slate-800 hover:border-red-500/40 rounded-xl p-4 text-center cursor-pointer transition bg-slate-950/40 relative">
          <input
            type="file"
          ...
          />
          <FaFileUpload className="mx-auto text-red-400 text-2xl mb-2" />
